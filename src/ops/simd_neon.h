...
static inline void simd_neon_bin_add(const float *a, const float *b, float *o, size_t n){
    size_t i=0;
    for(; i+4<=n; i+=4){ float32x4_t va=vld1q_f32(a+i); float32x4_t vb=vld1q_f32(b+i); float32x4_t vc=vaddq_f32(va,vb); vst1q_f32(o+i, vc); }
    for(; i<n; i++) o[i]=a[i]+b[i];
}
static inline void simd_neon_bin_sub(const float *a, const float *b, float *o, size_t n){
    size_t i=0;
    for(; i+4<=n; i+=4){ float32x4_t va=vld1q_f32(a+i); float32x4_t vb=vld1q_f32(b+i); float32x4_t vc=vsubq_f32(va,vb); vst1q_f32(o+i, vc); }
    for(; i<n; i++) o[i]=a[i]-b[i];
}
static inline void simd_neon_bin_mul(const float *a, const float *b, float *o, size_t n){
    size_t i=0;
    for(; i+4<=n; i+=4){ float32x4_t va=vld1q_f32(a+i); float32x4_t vb=vld1q_f32(b+i); float32x4_t vc=vmulq_f32(va,vb); vst1q_f32(o+i, vc); }
    for(; i<n; i++) o[i]=a[i]*b[i];
}
static inline void simd_neon_bin_div(const float *a, const float *b, float *o, size_t n){
    size_t i=0;
    for(; i+4<=n; i+=4){ float32x4_t va=vld1q_f32(a+i); float32x4_t vb=vld1q_f32(b+i); float32x4_t vc=vdivq_f32(va,vb); vst1q_f32(o+i, vc); }
    for(; i<n; i++) o[i]=a[i]/b[i];
}
static inline void simd_neon_scalar_add(const float *a, float bv, float *o, size_t n){
    float32x4_t vb=vdupq_n_f32(bv);
    size_t i=0;
    for(; i+4<=n; i+=4){ float32x4_t va=vld1q_f32(a+i); float32x4_t vc=vaddq_f32(va,vb); vst1q_f32(o+i, vc); }
...
static inline void simd_neon_relu(float *d, size_t n){
    ...
    size_t i=0;
    for(; i+4<=n; i+=4){ float32x4_t v=vld1q_f32(d+i); v=vmaxq_f32(v,z); vst1q_f32(d+i, v); }
