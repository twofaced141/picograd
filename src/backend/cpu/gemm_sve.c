...
void hgemm_sve_micro(size_t k, const uint16_t *a, size_t lda, const uint16_t *b, size_t ldb, float *c, size_t ldc, size_t m, size_t n){
    ...
    for(size_t j=0; j<n; ){
        ...
        for(size_t p=0;p<k;p++){
            ...
            if(m>0){ float av=pg_f16_to_f32_scalar(a[0*lda+p]); svfloat32_t va=svdup_n_f32(av); acc0=svmla_f32_x(pg, acc0, bf, va); }
            if(m>1){ float av=pg_f16_to_f32_scalar(a[1*lda+p]); svfloat32_t va=svdup_n_f32(av); acc1=svmla_f32_x(pg, acc1, bf, va); }
            if(m>2){ float av=pg_f16_to_f32_scalar(a[2*lda+p]); svfloat32_t va=svdup_n_f32(av); acc2=svmla_f32_x(pg, acc2, bf, va); }
            if(m>3){ float av=pg_f16_to_f32_scalar(a[3*lda+p]); svfloat32_t va=svdup_n_f32(av); acc3=svmla_f32_x(pg, acc3, bf, va); }
            if(m>4){ float av=pg_f16_to_f32_scalar(a[4*lda+p]); svfloat32_t va=svdup_n_f32(av); acc4=svmla_f32_x(pg, acc4, bf, va); }
            if(m>5){ float av=pg_f16_to_f32_scalar(a[5*lda+p]); svfloat32_t va=svdup_n_f32(av); acc5=svmla_f32_x(pg, acc5, bf, va); }
            if(m>6){ float av=pg_f16_to_f32_scalar(a[6*lda+p]); svfloat32_t va=svdup_n_f32(av); acc6=svmla_f32_x(pg, acc6, bf, va); }
            if(m>7){ float av=pg_f16_to_f32_scalar(a[7*lda+p]); svfloat32_t va=svdup_n_f32(av); acc7=svmla_f32_x(pg, acc7, bf, va); }
        }
        if(m>0) svst1_f32(pg, c + 0*ldc + j, acc0);
        if(m>1) svst1_f32(pg, c + 1*ldc + j, acc1);
        if(m>2) svst1_f32(pg, c + 2*ldc + j, acc2);
    ...
    if(m<=8 && n<=16){
        for(size_t j=0; j<n; ){
            ...
            for(size_t p=0;p<k;p++){
                ...
                if(m>0){ float av=pg_bf16_to_f32_scalar(a[0*lda+p]); svfloat32_t va=svdup_n_f32(av); acc0=svmla_f32_x(pg, acc0, bf, va); }
                if(m>1){ float av=pg_bf16_to_f32_scalar(a[1*lda+p]); svfloat32_t va=svdup_n_f32(av); acc1=svmla_f32_x(pg, acc1, bf, va); }
                if(m>2){ float av=pg_bf16_to_f32_scalar(a[2*lda+p]); svfloat32_t va=svdup_n_f32(av); acc2=svmla_f32_x(pg, acc2, bf, va); }
                if(m>3){ float av=pg_bf16_to_f32_scalar(a[3*lda+p]); svfloat32_t va=svdup_n_f32(av); acc3=svmla_f32_x(pg, acc3, bf, va); }
                if(m>4){ float av=pg_bf16_to_f32_scalar(a[4*lda+p]); svfloat32_t va=svdup_n_f32(av); acc4=svmla_f32_x(pg, acc4, bf, va); }
                if(m>5){ float av=pg_bf16_to_f32_scalar(a[5*lda+p]); svfloat32_t va=svdup_n_f32(av); acc5=svmla_f32_x(pg, acc5, bf, va); }
                if(m>6){ float av=pg_bf16_to_f32_scalar(a[6*lda+p]); svfloat32_t va=svdup_n_f32(av); acc6=svmla_f32_x(pg, acc6, bf, va); }
                if(m>7){ float av=pg_bf16_to_f32_scalar(a[7*lda+p]); svfloat32_t va=svdup_n_f32(av); acc7=svmla_f32_x(pg, acc7, bf, va); }
            }
            if(m>0) svst1_f32(pg, c+0*ldc+j, acc0);
            if(m>1) svst1_f32(pg, c+1*ldc+j, acc1);
            if(m>2) svst1_f32(pg, c+2*ldc+j, acc2);
